...
subprocess.run(['rm', '-rf', 'labels/'+run_mode])
subprocess.run([ 'mkdir', 'labels/'+run_mode])
 
subprocess.run(['rm', '-rf', 'images/'+run_mode])
subprocess.run([ 'mkdir', 'images/'+run_mode])
...
#     subprocess.run([ 'mkdir', 'images'+'/'+run_mode+'/'+class_name])
    ...
    for line in class_annotations:
        ...
        command = 'aws s3 --no-sign-request --only-show-errors cp s3://open-images-dataset/'+run_mode+'/'+line_parts[0]+'.jpg '+ 'images'+'/'+run_mode+'/'+line_parts[0]+'.jpg'
        commands.append(command)
        
 
        with open('labels/'+run_mode+'/%s.txt'%(line_parts[0]),'a') as f:
